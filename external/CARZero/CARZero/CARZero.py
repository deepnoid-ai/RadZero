...
def get_similarities(CARZero_model, imgs, txts, similarity_type="both"):
    ...
    if type(txts) == str or type(txts) == list:
        raise RuntimeError(
            "Text input not processed - please use CARZero_model.process_text"
        )
    if type(imgs) == str or type(imgs) == list:
        raise RuntimeError(
            "Image input not processed - please use CARZero_model.process_img"
        )
    ...
    if similarity_type == "global":
        ...
        attention_maps = torch.from_numpy(
            attention_maps.repeat(16, axis=1).repeat(16, axis=2)
        )  # Final
...
def get_query_similarities(CARZero_model, imgs, txts, similarity_type="both"):
    ...
    with torch.no_grad():
        ...
        label_img_emb_l = label_img_emb_l.view(
            label_img_emb_l.size(0), label_img_emb_l.size(1), -1
        )
...
def get_dqn_cos_similarities(CARZero_model, imgs, txts, similarity_type="both"):
    ...
    with torch.no_grad():
        ...
        # for i in range(bs):
        label_img_emb_l_ = label_img_emb_l.view(
            label_img_emb_l.size(0), label_img_emb_l.size(1), -1
        )
        ...
        #
        proj_img, atten_i2t = CARZero_model.fusion_module(
        ...
        # #
        proj_text, atten_t2i = CARZero_model.fusion_module(
...
def get_dqn_similarities(CARZero_model, imgs, txts, similarity_type="both"):
    ...
    with torch.no_grad():
        ...
        for i in range(bs):
            ...
            query_emb_l_ = query_emb_l.view(
                query_emb_l.size(0), query_emb_l.size(1), -1
            )

            query_emb_l_ = query_emb_l_.permute(
                0, 2, 1
            )  # patch_num b dim # [97, 512, 768]
            ...
            #  mode='local',
            #
            i2t_cls = i2t_cls.squeeze(
                -1
            )  # use text as query, use image as k, v, so image batch size have not distrubed the result
...
def get_report_detection(
    CARZero_model, imgs, txts, img_path, index, text_len, similarity_type="both"
):
    """Load a CARZero pretrained classification model
    ...
    with torch.no_grad():
        ...
        for i in range(bs):
            ...
            #
            i2t_cls, atten_i2t, it2_SimR = CARZero_model.fusion_module(
                torch.cat([label_img_emb_g_.unsqueeze(1), label_img_emb_l_], dim=1),
                query_emb_g[i * text_len : (i + 1) * text_len],
                return_atten=True,
                return_SimR=True,
            )
            # torch.cat([label_img_emb_g_.unsqueeze(1) , label_img_emb_l_], dim=1)
            ...
            # #
            t2i_cls, atten_t2i, t2i_SimR = CARZero_model.fusion_module(
                torch.cat(
                    [
                        query_emb_g[i * text_len : (i + 1) * text_len].unsqueeze(1),
                        query_emb_l_,
                    ],
                    dim=1,
                ),
...
def get_report_detection_similarities(
    CARZero_model, imgs, txts, similarity_type="both"
):
    """Load a CARZero pretrained classification model
    ...
    with torch.no_grad():
        ...
        for i in range(bs):
            # get similarities
            global_similarities = CARZero_model.get_global_similarities(
                img_emb_g[i : i + 1], text_emb_g[i * 2 : (i + 1) * 2]
            )
            local_similarities, attention_maps = CARZero_model.get_local_similarities(
                img_emb_l[i : i + 1],
                text_emb_l[i * 2 : (i + 1) * 2],
                txts["cap_lens"][i * 2 : (i + 1) * 2],
                return_atten=True,
            )
            similarities = (local_similarities + global_similarities) / 2
...
def plot_word_attention(tokens, attention_weights, output_filename):
    ...
    for b in range(batch_size):
        ...
        cax = ax.imshow(
            [attention_weights[b, 0, :sequence_length]], aspect="auto", cmap="hot"
        )
        ax.set_yticks([])
        ...
        plt.tight_layout()
        plt.savefig(
            f"{output_filename}{b}.png", dpi=300, format="png", bbox_inches="tight"
        )
        plt.close()
...
def gaussian_kernel(kernel_size=5, sigma=1.0):
    ...
    g = (1 / (2 * np.pi * sigma**2)) * torch.exp(
        -(coords**2 + coords.view(-1, 1) ** 2) / (2 * sigma**2)
    )
    return g / g.sum()
...
def apply_gaussian_blur_tensor(image_tensor, kernel):
    ...
    blurred_tensor = F.conv2d(
        image_tensor, kernel.unsqueeze(0).unsqueeze(0), padding=kernel.size(0) // 2
    )
...
def conve_attention_map(ws, batch_size):
    ...
    pred_map_expand = torch.from_numpy(
        pred_map.repeat(16, axis=2).repeat(16, axis=3)
    )  # Final
...
def get_grounding_map(CARZero_model, imgs, txts, similarity_type="both"):
    ...
    with torch.no_grad():
        ...
        #
        i2t_cls, atten_i2t = CARZero_model.fusion_module(
            torch.cat([label_img_emb_g.unsqueeze(1), label_img_emb_l_], dim=1),
            query_emb_g,
            return_atten=True,
        )
        ...
        # #
        t2i_cls, atten_t2i = CARZero_model.fusion_module(
            query_emb_l_, label_img_emb_g, return_atten=True
        )
...
def text_embedding(CARZero_model, cls_txt_mapping):
    with torch.no_grad():
        text_emb_l, text_emb_g, _ = CARZero_model.text_encoder_forward(
            cls_txt_mapping["caption_ids"],
            cls_txt_mapping["attention_mask"],
            cls_txt_mapping["token_type_ids"],
        )
...
def retriveal_shot_classification(CARZero_model, imgs, cls_txt_mapping, text_bs=256):
    ...
    cls_txt_mapping_ = [
        cls_txt_mapping_[i : i + text_bs] for i in range(0, total, text_bs)
    ]
    class_similarities = []
...
def report_detection_classification(
    CARZero_model, imgs, img_path, index, cls_txt_mapping
):
    """Load a CARZero pretrained classification model
    ...
    cls_similarity = get_report_detection(
        ...
        img_path,
        index,
        len(list(cls_txt_mapping[0].keys())),
    ...
    if len(list(cls_txt_mapping[0].keys())) == 2:
        ...
        columns_name = ["report"]
    class_similarities = pd.DataFrame(cls_similarity, columns=columns_name)
    return class_similarities
